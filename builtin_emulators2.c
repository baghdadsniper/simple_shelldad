...
int unset_alias(info_t *info, char *str)
{
	...
	*p = 0;
	ret = delete_node_at_index(&(info->alias),
		give_node_index(info->alias, node_start_with(info->alias, str, -1)));
	*p = c;
	return (ret);
}
...
int set_alias(info_t *info, char *str)
{
	...
	return (put_node_end(&(info->alias), str, 0) == NULL);
}
