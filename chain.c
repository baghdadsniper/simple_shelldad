...
/**
 * check_chain2 - checks we should continue chaining based on last status
 * @info: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
...
int replace_alias2(info_t *info)
{
	...
	for (i = 0; i < 10; i++)
	{
		node = node_starts_with2(info->alias, info->argv[0], '=');
		if (!node)
			return (0);
		free(info->argv[0]);
		p = _strchr(node->str, '=');
...
int replace_vars2(info_t *info)
{
	...
	for (i = 0; info->argv[i]; i++)
	{
		...
		{
			replace_string2(&(info->argv[i]),
							_strdup(convert_number2(info->status, 10, 0)));
			continue;
		}
		if (!_strcmp(info->argv[i], "$$"))
		{
			replace_string2(&(info->argv[i]),
							_strdup(convert_number2(getpid(), 10, 0)));
			continue;
		}
		node = node_starts_with2(info->env, &info->argv[i][1], '=');
		if (node)
		{
			replace_string2(&(info->argv[i]),
		...
		replace_string2(&info->argv[i], _strdup(""));
	}
	return (0);
}
