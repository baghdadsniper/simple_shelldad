	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			/* if (_strschr(*buf, ';')) is this a command chain? */
			{
				*len = r;
				info->cmd_buf = buf;
			}
...
ssize_t get_inpute(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		*buf_p = p; /* pass back pointer to current command position */
		return (_strslens(p)); /* return length of current command */
	}

	*buf_p = buf; /* else not a chain, pass back buffer from _getlinee() */
	return (r); /* return length of buffer from _getlinee() */
}
